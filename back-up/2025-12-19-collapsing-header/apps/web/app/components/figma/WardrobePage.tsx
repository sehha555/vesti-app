...
export function WardrobePage({ onNavigateToUpload, onNavigateToTryOn, onNavigateToBroadcast, savedOutfitsFromHome = EMPTY_OUTFITS, savedOutfitSetsFromTryOn = EMPTY_OUTFIT_SETS }: WardrobePageProps = {} as WardrobePageProps) {
  ...
  // 滾動監聽 - 使用 requestAnimationFrame 節流 + 死區
  useEffect(() => {
      ...
      rafRef.current = requestAnimationFrame(() => {
        ...
        if (scrollDelta >= 10) {
          ...
          if (currentScrollY > lastScrollYRef.current && currentScrollY > 50) {
            if (lastControlsStateRef.current !== false) {
          ...
          // 向上滾動 → 顯示控件
            ...
            if (lastControlsStateRef.current !== true) {
    ...
    <DndProvider backend={HTML5Backend}>
      <div className="h-screen flex flex-col bg-[var(--vesti-background)] overflow-hidden">
        ...
        <div className="flex-shrink-0 bg-[var(--vesti-background)]/95 backdrop-blur-sm">
          ...
          {/* 可隱藏的控制區域 - 固定高度防止 layout shift */}
          <motion.div
            animate={{
              y: isControlsVisible ? 0 : -120,
              opacity: isControlsVisible ? 1 : 0,
            }}
            transition={{ duration: 0.2, ease: [0.4, 0, 0.2, 1] }}
            className="relative overflow-hidden"
            style={{ willChange: 'transform, opacity', height: '120px' }}
          >
            {/* 視圖模式切換 */}
                    ...
                    {filteredOutfits.map((outfit, index) => {
                      ...
                      return (
                        <Reorder.Item
                          key={outfit.id}
                          value={outfit}
                          drag={!showDeleteButton}
                          dragListener={!showDeleteButton}
